...
class ResModConvBlock(ResidualBlock):
    def __init__(self, state, feats_in, c_in, c_mid, c_out, is_last=False):
        # 初始化方法，接受一系列参数，包括模块的状态 state、输入特征的数量 feats_in、
        # 输入通道数 c_in、中间通道数 c_mid、输出通道数 c_out 以及一个布尔值 is_last，用于表示是否是该模块的最后一个。
        skip = None if c_in == c_out else nn.Conv2d(c_in, c_out, 1, bias=False)
        # 创建一个 skip 连接（跳跃连接），如果输入通道数 c_in 等于输出通道数 c_out，
        # 则 skip 为 None；否则，创建一个 1x1 卷积层用于调整通道数。
        super().__init__([  # 调用父类 ResidualBlock 的初始化方法，传递了一系列子模块和 skip 连接。
